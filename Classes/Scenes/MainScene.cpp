...
namespace MonsterMaker { namespace Scenes{ namespace Mains{
	...
	bool MainLayer::init()
	{
		bool bRet = false;
		do{
			Utils::origin = Director::getInstance()->getVisibleOrigin();
			Utils::viewSize = Director::getInstance()->getVisibleSize();

			//************************************************************************/
			//* Super Init First
			//************************************************************************/
			CC_BREAK_IF(! LayerColor::initWithColor(Color4B(80,80,80,255)));
			...
			pMenu->setPosition(Point(winSize.width/2,winSize.height/2)); // 메뉴 위치 화면 가운데 지정 
			CC_BREAK_IF(!pMenu);
